...
function resetUI() {
    ...
    const neowsContent = docuement.getElementById('neows-content');
    const neows = document.querySelector('.card');

    console.log(apodContent);
    if (apodContent.length !== 0) {
        apodContent.removeChild(apod);
        neowsContent.removeChild(neows);
    }
    else {
        return;
    }
...
async function displayNeowsData() {
    ...
    if(date !== serverDate) {
        ...
        for (let i = 0; i < neows.near_earth_objects[serverDate].length; i++) {
            ...
            card.innerHTML = `
                <div class="card-body container">
                    <div class="row">
                        <h2 class="card-title mb-4 fw-bold">Neo Name: ${neows.near_earth_objects[serverDate][i].name}</h2>
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr>
                                <th class="fw-medium" scope="row">Absolute Magnitude</th>
                                <td colspan="3">${neows.near_earth_objects[serverDate][i].absolute_magnitude_h}</td>
                                </tr>
                                <tr>
                                <th class="fw-medium" scope="row">Minimum Estimated Diameter</th>
                                <td><em>KM:</em> ${neows.near_earth_objects[serverDate][i].estimated_diameter.kilometers.estimated_diameter_min} km </td>
                                <td><em>Mi:</em> ${neows.near_earth_objects[serverDate][i].estimated_diameter.miles.estimated_diameter_min} mi </td>
                                </tr>
                                <tr>
                                <th class="fw-medium" scope="row">Maximum Estimated Diameter</th>
                                <td><em>KM:</em> ${neows.near_earth_objects[serverDate][i].estimated_diameter.kilometers.estimated_diameter_max} km </td>
                                <td><em>KM:</em> ${neows.near_earth_objects[serverDate][i].estimated_diameter.miles.estimated_diameter_max} mi </td>
                                </tr>
                                <th class="fw-medium" scope="row">Potentially Hazardous</th>
                                <td colspan="3">${neows.near_earth_objects[serverDate][i].is_potentially_hazardous_asteroid}</td>
                                </tr>
                                <th class="fw-medium" scope="row">Close Approach Date</th>
                                <td colspan="3"><em>Close Approach Date:</em> ${neows.near_earth_objects[serverDate][i].close_approach_data[0].close_approach_date_full}</td>
                                </tr>
                            </tbody>
                        </table>
                        <p class="card-text">
                            <a href="${neows.near_earth_objects[serverDate][i].links.self}"> More information on ${neows.near_earth_objects[serverDate][i].name}</a>
                        </p>
                    </div>
                </div>
            `;
